...
> => ({
  getList: async ({ resource, pagination, filters, sorters, meta }) => {
    ...
    const { data, headers } = await httpClient[requestMethod]<ApiResponse<any>>(urlWithQuery, {
  ...
  create: async ({ resource, variables, meta }) => {
    ...
    const { data } = await httpClient[requestMethod]<ApiResponse<any>>(url, variables, {
      headers,
    });
  ...
  getOne: async ({ resource, id, meta }) => {
    ...
    const { data } = await httpClient[requestMethod]<ApiResponse<any>>(url, { headers });
